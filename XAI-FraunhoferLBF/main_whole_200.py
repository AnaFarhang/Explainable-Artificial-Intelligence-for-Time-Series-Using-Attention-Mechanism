...
class main:
    def __init__(self, epochs, learning_rate, n_layers, n_head, use_CWT, freq0_scale1, wavelet_type, desired_scales_freqs_range, selected_time_division, window_size, signal, fault_type):
        ...
        # Read healthy and faulty signals (give the proper directory)
        ...
        # We do wavelet feature extraction before the window division for both faulty signal
        log = 1
        from feature_extraction.wavelet_feature_extraction import wavelet_feature_extraction
        healthy_tensor = torch.tensor(df_healthy.values, dtype=torch.float32)
        healthy_tensor = healthy_tensor.view(1, 1, df_healthy.shape[0])
        healthy_data = wavelet_feature_extraction(healthy_tensor, sampling_rate, wavelet_type, freq0_scale1, desired_scales_freqs_range,1, log).data
        # We divide the transformed signal into time windows
        total_time_steps = healthy_data.shape[2]
        trimmed_len = total_time_steps - (total_time_steps % window_size)  # trims 24996 -> 249500
        healthy_data = healthy_data[:, :, :trimmed_len]  # [1, 200, 249500]
        healthy_data = healthy_data.reshape(-1, healthy_data.shape[1], window_size)

        # We do wavelet feature extraction before the window division for faulty signal
        faulty_tensor = torch.tensor(df_faulty.values, dtype=torch.float32)
        faulty_tensor = faulty_tensor.view(1, 1, df_faulty.shape[0])
        faulty_data = wavelet_feature_extraction(faulty_tensor, sampling_rate, wavelet_type, freq0_scale1, desired_scales_freqs_range, 1, log).data
        # We divide the transformed signal into time windows
        total_time_steps = faulty_data.shape[2]
        trimmed_len = total_time_steps - (total_time_steps % window_size)  # trims 24996 -> 249500
        faulty_data = faulty_data[:, :, :trimmed_len]  # [1, 200, 249500]
        faulty_data = faulty_data.reshape(-1, faulty_data.shape[1], window_size)

        # We concatenate the resulted signals
        tensors = [healthy_data, faulty_data]
        # To have the data with the same sizes for healthy and faulty samples, we find the minimum length and cut the signals
        min_len = min(t.shape[0] for t in tensors)
        data = torch.cat((healthy_data[:min_len], faulty_data[:min_len]), dim=0)
        data = data.to(device)
        ...
        for i in range(num_classes):
            all_labels.append(torch.tensor([i] * min_len))
        # Concatenate the tensors in the all_labels list
        labels = torch.cat(all_labels)

        # Choose every 8th member due to the computational issues
        data = data[:, :, ::selected_time_division]
        ...
        if use_CWT == 1 :
            cleaned_fault_type = fault_type.split('/')[0]
            for head in range(n_head):
                prefix = signal + '_attn_' + 'head' + str(head)
                attn_label = {}
                for i in range(num_classes):
                    attn_label[i] = attn_label_head[i][head]
                    with open('./explanation/' + prefix + 'L' + str(i)  + '_whole_window_size' + str(window_size)+ '_freq0_scale1_'+str(freq0_scale1) +'_'+cleaned_fault_type+'_200.pickle', 'wb') as f:
...
n_head = 1
n_layers = 2
# Here we give the address of the fault file that we want to analyze
        ...
        for window_size in [500, 50]:
            ...
            for wavelet_type in ['gaus4']:
                results = main(epochs=epochs, learning_rate=learning_rate, n_layers=n_layers, n_head=n_head, use_CWT=use_CWT, freq0_scale1= freq0_scale1, wavelet_type=wavelet_type,
                            desired_scales_freqs_range=desired_scales_freqs_range, selected_time_division=selected_time_division, window_size = window_size, signal=signal, fault_type = fault_type).result_dict
                ...
                df_result.to_csv('./'+"Franhoufer_whole_"+str(freq0_scale1)+'_'+cleaned_fault_type+"_200.csv", index=False, sep=';')
