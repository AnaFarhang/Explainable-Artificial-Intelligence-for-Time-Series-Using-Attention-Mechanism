...
class main:
    def __init__(self, epochs, learning_rate, n_layers, n_head, use_CWT, freq0_scale1, wavelet_type, desired_scales_freqs_range, selected_time_division, window_size, signal, fault_type):
        ...
        tensors = [health_tensor, fault_tensor]
        min_len = min(t.shape[0] for t in tensors)

        data = torch.cat((health_tensor[:min_len], fault_tensor[:min_len]), dim=0)
        data = data.to(device)
        ...
        num_classes = 2
        all_labels = []
        ## We need to have a list that assigns to each recorde it's label
        ...
        if use_CWT == 1 :
            ...
            for head in range(n_head):
                ...
                for i in range(num_classes):
                    ...
                    with open('./explanation/' + prefix + 'L' + str(i)  + '_window_size' + str(window_size)+ '_freq0_scale1_'+str(freq0_scale1) +'_'+cleaned_fault_type+'_200.pickle', 'wb') as f:
    ...
    for use_CWT in [1, 0]:
        ...
        for window_size in [500, 50]:
            ...
            for wavelet_type in ['gaus4']:
                ...
                df_result.to_csv('./'+"Franhoufer_"+str(freq0_scale1)+'_'+cleaned_fault_type+"_200.csv", index=False, sep=';')
