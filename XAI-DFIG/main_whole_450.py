...
class main:
    def __init__(self, epochs, learning_rate, n_layers, n_head, use_CWT, freq0_scale1, wavelet_type, desired_scales_freqs_range, selected_time_division, window_size, signal, REU, speed):
        ...
        # Apply wavelet feature extraction before window division for healthy data
        log = 1
        ...
        trimmed_len = total_time_steps - (total_time_steps % window_size)
        healthy_data = healthy_data[:, :, :trimmed_len]
        healthy_data = healthy_data.reshape(-1, healthy_data.shape[1], window_size)

        # Apply wavelet feature extraction before window division for faulty data
        ...
        trimmed_len = total_time_steps - (total_time_steps % window_size)
        faulty_data = faulty_data[:, :, :trimmed_len]
        faulty_data = faulty_data.reshape(-1, faulty_data.shape[1], window_size)

        data = torch.cat((healthy_data, faulty_data), dim=0)
        data = data.to(device)
        print('data', data.shape)

        num_classes = 2
        len_labels = [healthy_data.shape[0], faulty_data.shape[0]]

        all_labels = []
        for i in range(num_classes):
            all_labels.append(torch.tensor([i] * len_labels[i]))
        # Concatenate the tensors in the alllabels list
        ...
        def predict(model, loader):
            ...
            for batch in loader:
                ...
                all_attn_heads_layers = torch.cat((all_attn_heads_layers, attn_heads_layers[layer_number].to('cpu')),
                                                  dim=1)
        ...
        if use_CWT == 1 :
            for head in range(n_head):
                ...
                for i in range(num_classes):
                    ...
                    with open('./explanation/' + prefix + 'L' + str(i)  + '_whole_window_size' + str(window_size)+ '_freq0_scale1_'+str(freq0_scale1) +'_450.pickle', 'wb') as f:
                ...
                for speed in ['1590', '1560', '1530']:
                    ...
                    for wavelet_type in ['gaus4']:
                        results = main(epochs=epochs, learning_rate=learning_rate, n_layers=n_layers, n_head=n_head, use_CWT=use_CWT, freq0_scale1= freq0_scale1, wavelet_type=wavelet_type,
                                    desired_scales_freqs_range=desired_scales_freqs_range, selected_time_division=selected_time_division, window_size = window_size, signal=signal, REU = REU, speed = speed).result_dict
                        ...
                        all_results = {'epochs':[epochs], 'learning_rate':[learning_rate], 'n_layers':[n_layers], 'n_head':[n_head], 'use_CWT':[use_CWT], 'freq0_scale1': [freq0_scale1],
                         'wavelet_type':[wavelet_type], 'desired_scales_freqs_range':[desired_scales_freqs_range_string], 'selected_time_division':[selected_time_division],
                                       'window_size': [window_size], 'signal':[signal], 'REU':[REU], 'speed': [speed]}
                        all_results.update(results)
                        df_1 = pd.DataFrame(all_results)
                        df_result = pd.concat([df_result, df_1], axis = 0)
                        # Address to store the results
                        df_result.to_csv('./'+"DFIG_whole_"+str(freq0_scale1)+'_'+"_450.csv", index=False, sep=';')
